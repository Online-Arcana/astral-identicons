...
export const planetAnchorOuterRadius = 280;
export const planetGlyphSizes = [13, 15, 17, 19, 21, 23] as const;
export const planetDensityStrokeWidths = [0.2, 0.55, 0.9, 1.25, 1.6, 1.95] as const;
export const planetDensityOpacities = [0.68, 0.75, 0.82, 0.89, 0.95, 1] as const;
export const satelliteDotRadii = [1.2, 1.8, 2.5] as const;
export const satelliteOrbitPadding = 1.5;

export const centralSun = {
  medallionRadius: 60,
  glyphSize: 58,
  solarSigilKnockoutRadius: 10,
  solarSigilSize: 16,
  rayCount: 12,
  rayInnerRadius: 42,
  rayOuterRadius: 54,
...
export const parityStarSizes = [9, 13, 17, 21, 25, 29] as const;
export const parityDensityStrokeWidths = [0, 0.8, 1.6, 2.4, 3.2, 4] as const;
export const parityDensityOpacities = [0.64, 0.72, 0.8, 0.88, 0.94, 1] as const;
...
if (Math.max(...planetGlyphSizes) > Math.max(...parityStarSizes)) {
  throw new Error("v9 planetary glyphs must not exceed the largest parity star");
}
if (planetDensityStrokeWidths.length !== planetDensityLevelCount) {
  throw new Error("v9 planetary density geometry is inconsistent");
}
...
if (parityDensityStrokeWidths.length !== v9ParityDensityLevelCount) {
  throw new Error("v9 parity density geometry is inconsistent");
}
if (v9ParityPositionCount !== 8) {
  throw new Error("v9 parity groups require exactly eight local anchors");
}
if (planetAnchorSpacing <= maximumPlanetEnvelope * 2) {
  throw new Error("adjacent v9 planetary envelopes must not overlap");
}
if (planetAnchorInnerRadius <= centralSun.medallionRadius + maximumPlanetEnvelope) {
  throw new Error("v9 planetary anchors must clear the central Sun medallion");
}
