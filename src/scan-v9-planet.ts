...
interface DenseCandidate extends SizedCandidate {
...
function templateMask(
  ...
  const key = [glyph, rotation, sizeLevel, densityLevel].join(":");
  ...
  const stroke = planetDensityStrokeWidths[densityLevel]! * scale;

  context.clearRect(0, 0, canvas.width, canvas.height);
  context.translate(canvas.width / 2, canvas.height / 2);
  context.rotate(rotation * 30 * Math.PI / 180);
  context.globalAlpha = planetDensityOpacities[densityLevel]!;
  context.textAlign = "center";
  context.textBaseline = "middle";
  context.font = `600 ${size}px ${symbolFont}`;
  context.fillStyle = "#fff";
  context.strokeStyle = "#fff";
  context.lineWidth = stroke;
  context.lineJoin = "round";
  if (stroke > 0) context.strokeText(glyph, 0, 0);
...
function denseCandidates(
  ...
  const result: DenseCandidate[] = [];

  for (const candidate of sized) {
    for (let density = 0; density < planetDensityStrokeWidths.length; density += 1) {
      const expected = templateMask(
        glyph,
        candidate.rotation,
        candidate.size,
      ...
      const densityScore = bestCorrelation(candidate.observed, expected);
      result.push({
        ...candidate,
        density,
        densityScore,
        score: candidate.score * 0.35 + densityScore * 0.65
      });
    }
  }
...
function alternatives(
  ...
  const dense = denseCandidates(glyph, sized);
