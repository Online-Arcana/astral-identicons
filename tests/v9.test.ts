...
describe("v9 geometry and renderer", () => {
  test("provides 256 separated planetary anchors, 256 parity anchors and twelve Sun rays", () => {
    ...
    for (let anchor = 0; anchor < planetAnchorCount; anchor += 1) {
      ...
      for (let previous = 0; previous < anchor; previous += 1) {
        const other = planetAnchorPoint(previous);
    ...
    expect(minimumDistance).toBeGreaterThan(maximumPlanetEnvelope * 2);
  ...
  test("renders the specified glyph grammar and colour separation", async () => {
    ...
    expect(svg).toContain('data-recognition-role="literal-central-solar-sign"');
